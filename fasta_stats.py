...
outFile.write(",Std,")
outFile.write(str(np.std(np.array(lens)))+',')
outFile.write(str(np.std(np.array(GCs)))+',')
outFile.write(str(np.std(np.array(GC28s)))+',')
outFile.write(str(np.std(np.array(GGCs)))+',')
outFile.write(str(np.std(np.array(AAUs)))+'\n')
outFile.close()
...
# n, bins, patches = plt.hist(GC28s, normed=1, bins=50, color='c')
# # plt.hist(otherGCs, normed=1, bins=5, color='r', alpha = 0.5)
# y = mlab.normpdf(bins, mu, sigma)
# l = plt.plot(bins, y, 'b--', linewidth=2)
# plt.axvline(np.mean(arr), color='b', linestyle='dashed', linewidth=2)
# plt.show()
